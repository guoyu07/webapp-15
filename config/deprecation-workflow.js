...
window.deprecationWorkflow.config = {
  workflow: [
    ...
    { handler: "throw", matchMessage: "`Ember.Handlebars.registerBoundHelper` is deprecated. Please refactor to use `Ember.Helpers.helper`." },
    { handler: "throw", matchMessage: "In Ember 2.0 service factories must have an `isServiceFactory` property set to true. You registered (unknown mixin) as a service factory. Either add the `isServiceFactory` property to this factory or extend from Ember.Service." },
    { handler: "throw", matchMessage: "Ember.Select is deprecated. Consult the Deprecations Guide for a migration strategy." },
    { handler: "throw", matchMessage: "Ember.View is deprecated. Consult the Deprecations Guide for a migration strategy." },
    ...
    { handler: "throw", matchMessage: "In Ember Data 2.0, relationships will be asynchronous by default. You must set `user: DS.belongsTo('user', { async: false })` if you wish for a relationship remain synchronous." },
    { handler: "log", matchMessage: "Calling store.find() with a query object is deprecated. Use store.query() instead." },
    { handler: "log", matchMessage: "store.push(type, data) has been deprecated. Please provide a JSON-API document object as the first and only argument to store.push." },
    ...
    { handler: "log", matchMessage: "Using store.pushMany() has been deprecated since store.push() now handles multiple items. You should use store.push() instead." },
    { handler: "throw", matchMessage: "In Ember Data 2.0, relationships will be asynchronous by default. You must set `address: DS.belongsTo('address', { async: false })` if you wish for a relationship remain synchronous." },
    ...
    { handler: "throw", matchMessage: "In Ember Data 2.0, relationships will be asynchronous by default. You must set `photos: DS.hasMany('photo', { async: false })` if you wish for a relationship remain synchronous." }
  ]
};
